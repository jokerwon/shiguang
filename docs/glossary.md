...
### 操作卡片 (Action Card)
AI 执行写操作后,前端在消息流中渲染的结果卡片(如「已添加 牛腩 到食材清单」),附「撤销」按钮,撤销调同一 API 逆向操作。渲染数据随消息持久化,刷新后仍可显示。

### 待确认草稿 (Pending Draft)
偏好写工具的返回物:一份**未生效**的偏好变更。前端渲染确认卡片展示变更 diff,用户点「确认」后前端直接调偏好落库接口。**确认动作不经过 AI**,是确定性的 UI 路径(过敏原安全红线)。

### 会话 (Conversation) / 消息 (Message)
Phase 2 起新增的核心实体(ADR-0010)。一个用户可有多个会话;会话包含有序消息(用户/助手/工具三类角色)。持久化到服务端,刷新/换设备不丢。UI 为最小会话列表:切换 + 删除,不重命名/置顶/搜索。
